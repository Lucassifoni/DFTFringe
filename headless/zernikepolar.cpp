...
zernikePolar::zernikePolar(double rho, double theta, size_t nbTerms) {
    if (nbTerms > 49) {
        std::cerr << "zernikePolar::init: maxorder is limited to 49, setting to 49\n";
        nbTerms = 49;
    }
    ...
    double rho3, rho4, rho5, rho6, rho8, rho10;
    double cos3theta, sin3theta, cos4theta, sin4theta, cos5theta, sin5theta;
...
double zernikePolar::zernike(size_t n) {
    if (n < m_nbTermsComputed) {
        ...
        std::cerr << "zernikePolar: Zernike order " << n << " exceeds maximum computed order " << m_nbTermsComputed << "\n";
